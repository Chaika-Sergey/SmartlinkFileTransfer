...
CStdioFile		File("C:/test.txt", CFile::modeCreate|CFile::modeWrite|CFile::typeBinary);

BYTE			*pBufMemFileTra = (BYTE*) new char[357913941];
CMemFile		memFileTra(pBufMemFileTra, 357913941, 256);

BYTE			*pBufMemFilePriem = (BYTE*) new char[357913941];
CMemFile		memFilePriem(pBufMemFilePriem, 357913941, 256);


ULONG			lcFileSize = 0;


STDMETHODIMP Cs3_1000::PrepareFileToTransfer()
{
	CStdioFile		FileConfig("C:/SmartLinkConfigFileTra.ini", CFile::modeRead|CFile::typeBinary);
	CString			m_string;
	FileConfig.ReadString(m_string);
	m_string.Delete(m_string.GetLength() - 1, 1);
	
	CStdioFile		FileTra(m_string, CFile::modeRead|CFile::typeBinary);

	char			*bufMain;
	char			bufMainArea[1024*32];
	ULONG			i;

	memFileTra.SeekToBegin();
	bufMain = bufMainArea;
	i = 0;
	while (FileTra.Read(bufMain, 32768))
	{
		memFileTra.Write(bufMain, 32768);
		i++;
//		if (i == 1)		Fire_DisplayMessage(_bstr_t("Ôàéë ïîëó÷åí!"), MY_NAME);
	}

	lcFileSize = GetFileSize(FileTra, 0);
...
STDMETHODIMP Cs3_1000::WriteFileToDevice()
{
	char			bufArea[MAX_DMA_TRANSFER * 4 + 4096];
	char			*buf;
	ULONG			*bufULONG;
	char			*buf1;

	DWORD			i, transfer_size;
	...
	while ((unsigned long)buf1 % 0x1000 != 0)
		buf1++;
	...
	SetRegister(READ_ADDR_OFFSET, addr);

	SetRegister(READ_SIZE_OFFSET, 32);
	SetRegister(READ_COUNT_OFFSET, 256);
	...
	i = 0;
	transfer_size = 32768 - 16;

	parts_count = lcFileSize / transfer_size;
	...
	while (i <= lcFileSize)
	{
		...
		if (i + transfer_size <= lcFileSize)
		{
//			CopyMemory(buf+16, &charBuffer[i], transfer_size);

			memFileTra.Read(buf+16, transfer_size); 
			bufULONG = (ULONG*)buf;

			if (i == 0)
			{
				bufULONG[0] = (ULONG) lcFileSize;
			}
			else
			{
				bufULONG[0] = (ULONG) transfer_size;
			}

		}
		else
		{
//			CopyMemory(buf+16, &charBuffer[i], lcFileSize - transfer_size*part_count);

			memFileTra.Read(buf+16, transfer_size); 
			bufULONG = (ULONG*)buf;
			bufULONG[0] = (ULONG) lcFileSize - transfer_size*part_count;

		}

		part_count++;

		if (ReadFile(hs3_1000, buf, 32 * 4 * 256, &nTransfer, &asyncRead) == 0)
		{
...
STDMETHODIMP Cs3_1000::ReadFileFromDevice()
{

	char			bufArea[32768 + 4096];
	char			*buf;
	ULONG			*bufULONG;
	char			*buf1;
	...
	ULONG			addr;				// Address of memory buffer
	long			status;
	long			regValue;				// register value read
	ULONG			nOutput;				// bytes returned from IO ctrl call
	char			message[132];			// local mesage buffer

	ULONG			part_size;
	int			fullTime, fullTimeSec;
	SYSTEMTIME		time0;
	ULONG			expected_parts_count = 0;
	ULONG			expected_file_size = 0;
	...
	SetRegister(WRITE_ADDR_OFFSET, addr);
	SetRegister(WRITE_SIZE_OFFSET, 32);
	SetRegister(WRITE_COUNT_OFFSET, 256);
	...
	do
	{
		...
		if (WriteFile(hs3_1000, buf, 32 * 256 * 4, &nTransfer, &asyncWrite) == 0)		{		}
	...
	{	
		...
		{
			GetSystemTime(&time0);
			fullTime = time0.wMilliseconds;
			fullTimeSec = time0.wSecond;
		}
		...
		{
			bufULONG = (ULONG*)buf;
			expected_file_size = (ULONG)bufULONG[0];
		}

		memFilePriem.Write(buf+16, 32768-16);

/*
		bufULONG = (ULONG*)buf;
		part_size = (ULONG)bufULONG[0];

		memFilePriem.Write(buf+16, part_size);
		lcFileSize = lcFileSize + part_size;
*/
		part_count = part_count + 1;
		lcFileSize = lcFileSize + 32768 - 16;


	}
//	while (part_size == transfer_size);
	while (lcFileSize < expected_file_size);

	lcFileSize = expected_file_size;

	GetSystemTime(&time0);
	fullTime = time0.wMilliseconds - fullTime;
	fullTimeSec = time0.wSecond - fullTimeSec;
	fullTime = fullTime + fullTimeSec*1000;
...
/*
	char		str(11);
	char		*a = (char*) str;
	sprintf(a, "%d", fullTime);
*/
/*
	char		str(20);
	char* charBuffer = reinterpret_cast<char*>(fullTime );
...
*/
	...
	p = _itoa(speed, str1, radix);

	Fire_DisplayMessage(_bstr_t((LPCTSTR) p ), MY_NAME);
...
STDMETHODIMP Cs3_1000::SaveFileToDisk()
{

	CStdioFile		FileConfig("C:/SmartLinkConfigFilePriem.ini", CFile::modeRead|CFile::typeBinary);
	CString			m_string;
	FileConfig.ReadString(m_string);
	m_string.Delete(m_string.GetLength() - 1, 1);
	
	CStdioFile		FilePriem(m_string, CFile::modeCreate|CFile::modeWrite|CFile::typeBinary);



	FilePriem.Write((ULONG*)memFilePriem.Detach(), lcFileSize);
	memFilePriem.Close();
	FilePriem.Close();
...
long Cs3_1000::GetRegister(long offset, long *regValue)
{
